      ...
      <div className="pr-16">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">Sipariş: {mask(jobData.orderNumber)}</span>
          </div>
          {jobData.stokAdi !== undefined && (
            <div className="flex items-center gap-2">
              <Package size={16} className="text-purple-500" />
              <span className="text-sm">Stok: {mask(jobData.stokAdi)}</span>
            </div>
          )}
          <div className="flex items-center gap-2">
            <Package size={16} className="text-blue-500" />
            <span className="text-sm">Toplam: {jobData.totalQuantity ? parseInt(jobData.totalQuantity).toLocaleString() : 'N/A'}</span>
          </div>
          <div className="flex items-center gap-2">
            <Package size={16} className="text-green-500" />
            <span className="text-sm">Kalan: {jobData.remainingQuantity ? parseInt(jobData.remainingQuantity).toLocaleString() : 'N/A'}</span>
          </div>
          <div className="flex items-center gap-2">

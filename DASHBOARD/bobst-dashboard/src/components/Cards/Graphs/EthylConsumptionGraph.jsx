...
export default function EthylConsumptionGraph({ data, isDark = false, style, range = '24h', currentLanguage = 'tr' }) {
  ...
  const processedData = useMemo(() => {
    ...
    const cleanData = data
      .filter(point => {
        ...
        if (!point || !point.name) return false;
        
        // NaN ve Infinity değerleri filtrele
        const ethylAcetate = Number(point.ethylAcetate);
        const ethylAlcohol = Number(point.ethylAlcohol);
      ...
      .map(point => ({
        name: point.name,
        ethylAcetate: Number(point.ethylAcetate),
        ethylAlcohol: Number(point.ethylAlcohol),
    ...
    // Range'e göre maksimum nokta sayısı belirle - OPTIMIZE EDİLDİ
    let maxPoints;
    switch (range) {
      case '12h': maxPoints = 120; break; // 12 saat * 10 dakika gruplama - düşük çözünürlük
      case '24h': maxPoints = 144; break; // 24 saat * 10 dakika gruplama - düşük çözünürlük
      case '1w': maxPoints = 168; break; // 7 gün * 1 saat gruplama - düşük çözünürlük
      case '1m': maxPoints = 720; break; // 30 gün * 1 saat gruplama - düşük çözünürlük
      case '1y': maxPoints = 8760; break; // 365 gün * 1 saat gruplama - düşük çözünürlük
      default: maxPoints = 200;
    }
    ...
    for (let i = 0; i < cleanData.length; i += step) {
      ...
      if (i > 0) {
        ...
        // Daha az hassas değişim algılama - PERFORMANS İÇİN OPTIMIZE EDİLDİ
        const threshold = (range === '12h' || range === '24h') ? 0.15 : 0.20;
  ...
  const renderChart = (height = 350, showBrushComponent = false) => {
    // Veri kontrolü

...
export default function DieSpeedGraph({ data, isDark = false, style, range = '24h', lineColor = "#FF6B6B", currentLanguage = 'tr' }) {
  ...
  const gradientColors = useMemo(() => {
    if (isDark) {
      return {
        ...
        end: 'rgba(255, 107, 107, 0.1)'
      };
    }
    return {
      start: 'rgba(255, 107, 107, 0.6)',
  ...
  const tooltipProps = {
    ...
    formatter: (value) => [`${Math.round(value)} ${getTranslation('beatsPerMin', currentLanguage)}`, getTranslation('speed', currentLanguage)],
    separator: ": "
  };

  // İstatistikler
  const stats = useMemo(() => {
    if (!processedData || processedData.length === 0) return { avg: 0, max: 0, min: 0, current: 0 };
    ...
    try {
      ...
      if (values.length === 0) return { avg: 0, max: 0, min: 0, current: 0 };
  ...
  const renderChart = (height = 400, showBrushComponent = false) => {
    ...
    return (
      <ResponsiveContainer width="100%" height={height}>
        ...
        >
          <defs>
            <linearGradient id="dieSpeedGradient" x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis 
            domain={[0, 'auto']} 
            stroke={isDark ? "#d1d5db" : "#374151"}
            tickFormatter={(value) => `${Math.round(value)} ${getTranslation('beatsPerMin', currentLanguage)}`}
          />
          <CartesianGrid 
          ...
          <DataComponent
            ...
            dataKey="value"
            stroke={lineColor}
            fill={chartType === 'area' ? "url(#dieSpeedGradient)" : undefined}
            dot={false}
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">{getTranslation('dieSpeed', currentLanguage)}</h3>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          <div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
            <div className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.current}</div>
            <div className="text-xs text-red-600 dark:text-red-400">{getTranslation('current', currentLanguage)} ({getTranslation('beatsPerMin', currentLanguage)})</div>
          </div>
          <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">{stats.avg}</div>
            <div className="text-xs text-green-600 dark:text-green-400">{getTranslation('average', currentLanguage)} ({getTranslation('beatsPerMin', currentLanguage)})</div>
          </div>
          <div className="text-center p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
            <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">{stats.max}</div>
            <div className="text-xs text-orange-600 dark:text-orange-400">{getTranslation('maximum', currentLanguage)} ({getTranslation('beatsPerMin', currentLanguage)})</div>
          </div>
          <div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
            <div className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.min}</div>
            <div className="text-xs text-red-600 dark:text-red-400">{getTranslation('minimum', currentLanguage)} ({getTranslation('beatsPerMin', currentLanguage)})</div>
          </div>
        </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-2xl font-bold">{getTranslation('dieSpeed', currentLanguage)} - {getTranslation('fullScreenView', currentLanguage)}</h3>
              <button
                onClick={() => setIsZoomed(false)}
